...
params$time_grid <- 7
params$tgrid_min <- c(49, 3)   # summer bookend: May...... ;    August.......;
params$tgrid_max <- c(52, 6)   # winter bookend: 04Dec-31Dec=weeks49-52; 15Jan-14Jun = weeks3-6;   
...
params$species_to_process <- c("amerob")
...
p <- grid.arrange(grobs = plots, ncol = 10, nrow=11)

# ggsave("~/Documents/plots/amerob/surv_prod_flact_wint_book.png", plot=p, width = 60, height= 40, units = "cm")  
...
for(species_code in params$species_to_process){
  ...
  fl <- 6.5 # a fixed scale to enable comparison with other sp!
...
# ggsave("~/Documents/plots/amerob/VarDiff_survProd_wint_book_unsmooth.png", plot=p, width = 40, height= 20, units = "cm")  
      ...
      if(nrow(data_regression)>min_n){
        ...
        if(converged){
          ...
          output=data.frame(cell=cells_all[i],
                            ...
                            p_value=mean(slopes > 0), # what if slope is negative?
                            converged=converged
          )
        } else{
...
# ggsave("~/Documents/plots/amerob/wintT_surv_wint_book_unsmooth.png", plot=p, width = 30, height= 20, units = "cm")  

plot_regression(data_regression, "swe", "mean", params$plotting_xlim, fill_lim = c(-1,1))+
  theme(legend.text = element_text(size=15),
        legend.title = element_text(size=20),
        legend.key.size = unit(1, "cm"))+labs(fill="effect size")
# ggsave("~/Documents/plots/amerob/swe_surv_wint_book_unsmooth.png", plot=p, width = 30, height= 20, units = "cm")
...
# ggsave("~/Documents/plots/amerob/wintT_surv_wint_book_Smooth_meanProb.png", plot=p2, width = 30, height= 20, units = "cm")  
...
gridExtra::grid.arrange(p1, p2, ncol=2)
# ggsave("~/Documents/plots/amerob/swe_surv_wint_book_Smooth_meanProb.png", plot=p2, width = 30, height= 20, units = "cm")  
...
theme(legend.text = element_text(size = 15), 
      legend.title = element_text(size = 20), 
      legend.key.size = unit(1, "cm"))+labs(fill='effect size') 
# ggsave("~/Documents/plots/amerob/varDiff_survProd_wint_book_Smoothed_meanProb.png", plot=p2, width = 30, height= 20, units = "cm")  
