...
p1 <- ggplot() + coord_fixed() + blank_theme +
  geom_sf(data=region_of_interest, fill=NA, color="black")   +
...
p2 <- ggplot() + coord_fixed() + blank_theme +
  geom_sf(data=region_of_interest, fill=NA, color="black")   +
...
###
###     Smoothing recruitment and mortality variancesc  ----------------------------------------
###

# prepare CAR model data from variance data

head(plotting_data, 2)
head(car_data_select, 2)
...
car_var_data <- 
  ...
  # remove a region where we have sparse data
  filter(!(lat > 38.7 & lon < -99.5)) %>%
  mutate(known_se= 1-2*abs(p_survival_variance_higher - 0.5))
...
macrodemography:::get_adjacency_matrix(car_var_data$cell, grid_large) -> adjacency_mat


# ICAR model without latitude
icar_fit <-
  brm(effect_size_log | resp_se(known_se, sigma = TRUE) ~
        car(M, gr = cell, type = "icar"),
      data = car_var_data,
      data2 = list(M = adjacency_mat),
      backend = 'cmdstanr',
...
npt <- nrow(car_var_data)
true_values <- matrix(nrow = 40000, ncol = npt)
for(i in 1:npt) {
  print(i)
  # the (unsmoothed) posterior slope:
  M <- car_var_data$effect_size_log[i]
  # the posterior uncertainty from model:
  se <- car_var_data$known_se[i]
  # get posterior draws of the linear predictor
  # re.form=NULL (default), i.e. include all group-level effects
  # incl_autor=TRUE (default), i.e. include correlation structures in prediction
  LP <-  posterior_linpred(icar_fit, re.form = NULL, incl_autocor = T)[ , i]
  # get draws of the unobserved latent uncertainty:
  sigma <- as_draws_df(icar_fit)$sigma
  # error propagation, combining sigma form CAR model and se from model
  # QUESTION: why the linear predictor here
  mu = (M*sigma^2 + LP * se^2)/(sigma^2 + se^2)
  scale = 1/sqrt(1/sigma^2 + 1/se^2)
  true_values[ , i] <- rnorm(40000, mu, scale)
...
smooth_prob <-
  data.frame(
    cell = car_var_data$cell,
    smooth_prob =
      apply(
        true_values, 2,
        function(x){
          mean(
            x +  mean(car_var_data$effect_size_log)/
              sd(car_var_data$effect_size_log) > 0
          )
        }
      )
  )
  ...
  data.frame(
    ...
    smooth_mean =
      apply(true_values, 2,
            function(x){
              mean(x) +
                mean(car_var_data$effect_size_log)/
                sd(car_var_data$effect_size_log
                )
            }
      )
  )

plotting_data3 <- merge(car_var_data, smooth_prob, by = "cell") |>
  merge(smooth_mean, by = "cell")
