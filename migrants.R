...
species <- data.frame(four = c("woth"),
                      six = c("woothr"))
...
for(sp in 1:nrow(species)){
    ...
    for (j in 1:nrow(effort_lim)) {
      ...
      for (y in seq_along(years)) {
        ...
        spring_data[[i]][[j]][[y]] <- get_grid_data(data = spd3, .year = years[y], 
                                                    tgrid_min = 21, tgrid_max = 22, 
                                                    min_lat = 25, max_lat = 50, min_lon = -102)
        fall_data[[i]][[j]][[y]] <- get_grid_data(data = spd3, .year = years[y], 
                                                  tgrid_min = 30, tgrid_max = 31, 
                                                  min_lat = 25, max_lat = 50, min_lon = -102)
      }
    }
  }
  
  saveRDS(spring_data, paste0("macrodemography/migrants/", species$four[sp], "/spring_data.RDS"))
  saveRDS(fall_data, paste0("macrodemography/migrants/", species$four[sp], "/fall_data.RDS"))
  ...
  saveRDS(spring_abun_data, paste0("macrodemography/migrants/", species$four[sp], "/spring_abun_data.RDS"))
  saveRDS(fall_abun_data,  paste0("macrodemography/migrants/", species$four[sp], "/fall_abun_data.RDS"))
}
...
spring_abun_data <- readRDS("macrodemography/migrants/woth/spring_abun_data.RDS")
fall_abun_data <- readRDS("macrodemography/migrants/woth/fall_abun_data.RDS")


abun_data_bycell <- function(abun_data, min_n_timeslice = 4) {
...
for (y in seq_along(years)) {
  print(y)
  spring_abun_summary[[y]] <- abun_data_bycell(spring_abun_data[[y]], min_n_timeslice = 2)
  fall_abun_summary[[y]] <- abun_data_bycell(fall_abun_data[[y]], min_n_timeslice = 2)
}
...
for (i in 1:length(cell_ratio_series)) {
  if(sum(is.na(cell_ratio_series[[i]]$median)) < 5){
    plot(cell_ratio_series[[i]]$median, ylim = c(-5,5), pch = 16, main = cells[i], col = c("blue", rep(c("red", "blue"), 7)))
...
p <- ggplot() + 
  ...
  geom_polygon(data=grid_2,      aes(x=long, y=lat, group=group, fill=avg_sd_diff_pos), alpha=0.4)    +
...
#######
plotting_data2 <- data.frame(cell = cells, b_janfeb = NA, pd_janfeb = NA, b_jul = NA, pd_jul = NA,
                             b_jul_fullyr = NA, pd_jul_fullyr = NA, b_jul_fullyr_1 = NA, pd_jul_fullyr_1 = NA)

for (i in seq_along(cells)) {
  b_dist <- vector()
  if(!is.null(janfeb_temp[[i]]) & sum(!is.na(cell_ratio_series_full[[i]][2*c(1:7), 1])) > 4 & 
     !(Inf %in% cell_ratio_series_full[[i]][2*c(1:7), ]) & !(-Inf %in% cell_ratio_series_full[[i]][2*c(1:7), ])) {
    for (j in 1:100) {
      b_dist[j] <- lm(cell_ratio_series_full[[i]][2*c(1:7), j] ~ unlist(janfeb_temp[[i]][1:7]))$coefficients[2]
    }
    plotting_data2$b_janfeb[i] <- mean(b_dist)
    plotting_data2$pd_janfeb[i] <- sum((b_dist > 0) == (mean(b_dist) > 0))
  }
  
  if(!is.null(janfeb_temp[[i]]) & sum(!is.na(cell_ratio_series_full[[i]][1 + 2*c(1:7), 1])) > 4 &
     !(Inf %in% cell_ratio_series_full[[i]][1 + 2*c(1:7), ]) & !(-Inf %in% cell_ratio_series_full[[i]][1 + 2*c(1:7), ])) {
    b_dist <- vector()
    for (j in 1:100) {
      b_dist[j] <- lm(cell_ratio_series_full[[i]][1 + 2*c(1:7), j] ~ unlist(jul_temp[[i]][2:8]))$coefficients[2]
    }
    plotting_data2$b_jul[i] <- mean(b_dist)
    plotting_data2$pd_jul[i] <- sum((b_dist > 0) == (mean(b_dist) > 0))
  }
  
  if(!is.null(janfeb_temp[[i]]) & sum(!is.na(cell_ratio_series_full[[i]][2*c(1:7), 1] + cell_ratio_series_full[[i]][1 + 2*c(1:7), 1])) > 4 &
     !(Inf %in% (cell_ratio_series_full[[i]][2*c(1:7), 1] + cell_ratio_series_full[[i]][1 + 2*c(1:7), 1])) & 
     !(-Inf %in% (cell_ratio_series_full[[i]][2*c(1:7), 1] + cell_ratio_series_full[[i]][1 + 2*c(1:7), 1]))) {
    b_dist <- vector()
    for (j in 1:100) {
      b_dist[j] <- lm(cell_ratio_series_full[[i]][2*c(1:7), j] + cell_ratio_series_full[[i]][1 + 2*c(1:7), j] ~ unlist(jul_temp[[i]][2:8]))$coefficients[2]
    }
    plotting_data2$b_jul_fullyr[i] <- mean(b_dist)
    plotting_data2$pd_jul_fullyr[i] <- sum((b_dist > 0) == (mean(b_dist) > 0))
  }
  ...
  if(!is.null(janfeb_temp[[i]]) & sum(!is.na(cell_ratio_series_full[[i]][2*c(1:7), 1] + cell_ratio_series_full[[i]][-1 + 2*c(1:7), 1])) > 4 &
    ...
    b_dist <- vector()
    for (j in 1:100) {
      b_dist[j] <- lm(cell_ratio_series_full[[i]][2*c(1:7), j] + cell_ratio_series_full[[i]][-1 + 2*c(1:7), j] ~ unlist(jul_temp[[i]][1:7]))$coefficients[2]
    }
    plotting_data2$b_jul_fullyr_1[i] <- mean(b_dist)
    plotting_data2$pd_jul_fullyr_1[i] <- sum((b_dist > 0) == (mean(b_dist) > 0))
  }
  
}

plotting_data2$pp_janfeb <- plotting_data2$pd_janfeb * (plotting_data2$b_janfeb > 0) +
  (100 - plotting_data2$pd_janfeb) * (plotting_data2$b_janfeb < 0)

plotting_data2$pp_jul <- plotting_data2$pd_jul * (plotting_data2$b_jul > 0) +
  (100 - plotting_data2$pd_jul) * (plotting_data2$b_jul < 0)

plotting_data2$pp_jul_fullyr <- plotting_data2$pd_jul_fullyr * (plotting_data2$b_jul_fullyr > 0) +
  (100 - plotting_data2$pd_jul_fullyr) * (plotting_data2$b_jul_fullyr < 0)

plotting_data2$pp_jul_fullyr_1 <- plotting_data2$pd_jul_fullyr_1 * (plotting_data2$b_jul_fullyr_1 > 0) +
  (100 - plotting_data2$pd_jul_fullyr_1) * (plotting_data2$b_jul_fullyr_1 < 0)

grid_3  <- merge(grid_2,plotting_data2,by.x="cell")

p <- ggplot() + 
  geom_polygon(data=states, aes(x=long, y=lat, group=group), fill=NA, color="black")   +
  geom_polygon(data=grid_3,      aes(x=long, y=lat, group=group, fill=pp_janfeb), alpha=0.4)    +
  geom_path   (data=grid_3,      aes(x=long, y=lat, group=group), alpha=0.4, color="white") +
  viridis::scale_fill_viridis()
p

p <- ggplot() + 
  geom_polygon(data=states, aes(x=long, y=lat, group=group), fill=NA, color="black")   +
  geom_polygon(data=grid_3,      aes(x=long, y=lat, group=group, fill=pp_jul), alpha=0.4)    +
  geom_path   (data=grid_3,      aes(x=long, y=lat, group=group), alpha=0.4, color="white") +
  viridis::scale_fill_viridis()
p
...
p <- ggplot() + 
  geom_polygon(data=states, aes(x=long, y=lat, group=group), fill=NA, color="black")   +
  geom_polygon(data=grid_3,      aes(x=long, y=lat, group=group, fill=pp_jul_fullyr), alpha=0.4)    +
  geom_path   (data=grid_3,      aes(x=long, y=lat, group=group), alpha=0.4, color="white") +
  viridis::scale_fill_viridis()
p
...
p <- ggplot() + 
  geom_polygon(data=states, aes(x=long, y=lat, group=group), fill=NA, color="black")   +
  geom_polygon(data=grid_3,      aes(x=long, y=lat, group=group, fill=pp_jul_fullyr_1), alpha=0.4)    +
  geom_path   (data=grid_3,      aes(x=long, y=lat, group=group), alpha=0.4, color="white") +
  viridis::scale_fill_viridis()
p
