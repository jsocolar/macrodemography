...
params:
  ...
  n_variance_min: 5 # minimum number of years to calculate a variance for a cell
...
for(species_code in params$species_to_process){
  ...
  if(params$always_run_variance_test | !file.exists(var_save_path)){
    ...
    data_compare_ratios <- lapply(sort(unique(tidy_ratios$summary$cell)),
                                  ...
                                  n_ratio_min=params$n_variance_min,
...
for(species_code in params$species_to_process){
  ...
  plotting_data <- cell_lrat_sd |>
    rename(
      `p(survival)` = p_survival_variance_higher,
      surv_sd_diff = effect_size_log
      )
  
   tidy_ratios$summary[,c("cell","n_prod","n_surv","n")] %>% group_by(cell) %>% arrange(cell) %>% filter(row_number()==1) %>% right_join(cell_lrat_sd, by="cell")
...
tidy_ratios$summary %>% 
  ...
  filter(n_prod >= params$n_variance_min & n_surv >= params$n_variance_min) %>%
...
dggridR::dgcellstogrid(
  ...
  # define color scales
  scale_viridis <- viridis::scale_fill_viridis(limits = c(params$n_variance_min, length(params$years) - 1))
